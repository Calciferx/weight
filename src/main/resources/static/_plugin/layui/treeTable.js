...
/*
* 改: 所有用data()获取属性值的地方均获取不到, 需改成attr('data-xxx')形式[已改部分]
* */
...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.init = function () {
        ...
        function patchHide($tr) {
            ...
            var parentKey = $tr.attr('data-parent'), pCol;
            if (!parentKey) return;
    ...
    TreeTable.prototype.bindEvents = function () {
            ...
            var obj = {
                ...
                del: function () { // 删除行
                    ...
                    $tr.nextAll('tr').each(function () {
                        ...
                        if (parseInt($(this).attr('data-indent')) <= indent) return false;
                        $(this).remove();
                    });
                    // 更新后面同辈的index
                    ...
                    $pTr.each(function () {
                        ...
                        var tInd = parseInt($(this).attr('data-indent'));
        ...
        $allBody.off('click.fold').on('click.fold', '.ew-tree-pack', function (e) {
            ...
            var haveChild = $tr.attr('data-have-child');
                ...
                if ('text' === edit || 'number' === edit) {  // 文本框
                    ...
                    $input.blur(function () {
                        ...
                        if (rs === false) {
                            ...
                            var keys = $td.attr('data-key').split('-');
    ...
    TreeTable.prototype.renderBodyData = function (data, d, $tr) {
        ...
        if ($tr) {
            ...
            indent = parseInt($tr.attr('data-indent')) + 1;
        ...
        if ($tr) {
            ...
            $tr.nextAll('tr').each(function () {
                ...
                if (parseInt($(this).attr('data-indent')) <= (indent - 1)) return false;
                $(this).remove();
            });
            // 渲染新dom
    ...
    TreeTable.prototype.renderBodyTr = function (d, indent, hide, $tr) {
        ...
        if ($tr) {
            ...
            $tr.attr('data-have-child', haveChild ? 'true' : 'false');
            $tr.attr('data-indent', indent);
            $tr.removeClass('ew-tree-table-loading');
        }
        var html = '<tr';
        var classNames = '';
    ...
    TreeTable.prototype.getDataByTr = function ($tr) {
        ...
        if (typeof $tr !== 'string' && typeof $tr !== 'number') {
            if ($tr) index = $tr.attr('data-index');
        } else index = $tr;
        ...
        for (var i = 0; i < index.length; i++) {
            if (data) {
                data = data[this.options.tree.childName][index[i]];
            } else {
                data = this.options.data[index[i]];
            }
        }
        return data;
    };
    ...
    TreeTable.prototype.checkSubCB = function ($tr, checked) {
        ...
        if ($tr.is('tbody')) {
            ...
            indent = parseInt($tr.attr('data-indent'));
    ...
    TreeTable.prototype.checkParentCB = function ($tr) {
        ...
        if (ckNum > 0 && unCkNum === 0) {  // 全选
            ...
            $cb.attr('data-indeterminate', 'true');
        ...
        if ('show' === type) {
            ...
            components.$tBody.children('tbody').children('tr').each(function () {
                ...
                $(this).children('td').each(function () {
                    ...
                    if ($this.attr('data-type') !== 'normal' || $this.hasClass('layui-hide')) return true;
    ...
    TreeTable.prototype.printTable = function () {
        ...
        function hideCol($temp) {
            ...
            var parentKey = $temp.attr('data-parent'), pCol;
            if (!parentKey) return;
    ...
    TreeTable.prototype.toggleCol = function (show, field, key) {
        ...
        if (show === undefined) {
            ...
            this.eachCols(function (i, item) {
                ...
                cols.push('<li><input type="checkbox" lay-skin="primary"></li>');
    ...
    function toggleRow($tr) {
        ...
        if (open) {  // 折叠
            ...
            $tr.nextAll('tr').each(function () {
                ...
                var ind = parseInt($(this).attr('data-indent'));
                ...
                if (!$(this).hasClass('ew-tree-table-open')) hideInd = parseInt($(this).attr('data-indent'));
    ...
    $(document).on('mousedown', '.ew-tb-resize', function (e) {
        ...
        // var x = $rs.data('x');
        // var w = $rs.data('width');
        var x = $rs.attr('data-x');
        var w = $rs.attr('data-width');
        ...
        var key = $rs.parent().attr('data-key');
        var ks = key.split('-');
